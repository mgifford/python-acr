# 🛡️ Accessibility Conformance Report (ACR) Generator
...
## Adding Scan Results & Making Them Visible in the Comparator
 ...
 - **Place the scan directory**: copy your scan folder into the `results/` directory. Example folder name used in this repo: `drupal-default-12-21-2025` (contains files like `issues_raw_20251221.csv`, `issues_summarized_20251221.csv`).
 - **Update the index**: add a reference to the relevant CSV in `results/index.json`. Example entry:

```
"results/drupal-default-12-21-2025/issues_raw_20251221.csv"
```

 - **Rebuild comparator JSON**: run the build script (no parameters needed):

```bash
python build_comparator_json.py
```

 - **Commit & push**: stage the new folder and `results/index.json`, commit and push to GitHub:

```bash
git add results/drupal-default-12-21-2025 results/index.json
git commit -m "Add drupal-default-12-21-2025 scan and index entry"
git push origin main
```

After the push, the `comparison.html` tool will include the new scan when it loads `results/comparison.json` (regenerated by the build script).
